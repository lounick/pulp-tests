...
for i in range(ndepots):
    if i == 0:
        m.addConstr(quicksum(vars[i, j] for j in range(ndepots, len(points))) == 0)
    else:
...
for j in range(ndepots):
    if j == 0:
        m.addConstr(quicksum(vars[i, j] for i in range(ndepots, len(points))) == salesmen)
    else:
        m.addConstr(quicksum(vars[i, j] for i in range(ndepots, len(points))) == 0)
m.update()
